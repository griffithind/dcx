...
// GetWorkspaceFolder returns the workspace folder path inside the container.
// This is a convenience method for the ContainerRuntime interface.
func (r *ResolvedDevContainer) GetWorkspaceFolder() string {
	return r.WorkspaceFolder
}

// GetContainerName returns the primary container name.
// This is a convenience method for the ContainerRuntime interface.
func (r *ResolvedDevContainer) GetContainerName() string {
	return r.ServiceName
}

// GetBuildLabels returns labels to apply during container creation.
func (r *ResolvedDevContainer) GetBuildLabels(dcxVersion string) *state.ContainerLabels {
	l := state.NewContainerLabels()

	// Identity
	l.WorkspaceID = r.ID
	l.WorkspaceName = r.Name
	l.WorkspacePath = r.LocalRoot
	l.ConfigPath = r.ConfigPath

	// Hashes
	if r.Hashes != nil {
		l.HashConfig = r.Hashes.Config
		l.HashDockerfile = r.Hashes.Dockerfile
		l.HashCompose = r.Hashes.Compose
		l.HashFeatures = r.Hashes.Features
		l.HashOverall = r.Hashes.Overall
	}

	// State
	l.CreatedAt = time.Now()
	l.CreatedBy = dcxVersion
	l.LifecycleState = state.LifecycleStateCreated

	// Build info
	if r.Plan != nil {
		l.BuildMethod = string(r.Plan.Type())
	}
	l.BaseImage = r.BaseImage
	l.DerivedImage = r.DerivedImage

	// Compose
	if compose, ok := r.Plan.(*ComposePlan); ok {
		l.ComposeProject = compose.ProjectName
		l.ComposeService = compose.Service
	}

	// Features
	if len(r.Features) > 0 {
		featureIDs := make([]string, len(r.Features))
	...
	// Cache
	if r.Hashes != nil {
		l.CacheData = &state.CacheData{
			ConfigHash:     r.Hashes.Config,
			DockerfileHash: r.Hashes.Dockerfile,
			ComposeHash:    r.Hashes.Compose,
			FeaturesHash:   r.Hashes.Features,
